...
local mathfloor, mathabs, mathcos, mathsin, mathrad, mathclamp = math.floor, math.abs, math.cos, math.sin, math.rad, math.clamp or function(x, a, b) return math.max(a, math.min(b, x)) end
...
local function Wrap(Player)
    ...
    local D = {
        ...
        HBMain=Drawingnew("Square"),
        HBOutline=Drawingnew("Square"), 
        Box2D=Drawingnew("Square"),
    }
    ...
    Entry.Connections.Render = RunService.RenderStepped:Connect(function()
        ...
        if S.TeamCheck and Player.Team and LocalPlayer.Team and Player.Team == LocalPlayer.Team then HideAll(); return end
        local maxD=S.MaxDistance
        if maxD and maxD>0 and GetDistance(HRP.Position)>maxD then HideAll(); return end
        ...
        if HB and HB.Enabled then
            local ratio=mathclamp(Hum.Health/math.max(Hum.MaxHealth,1),0,1)
